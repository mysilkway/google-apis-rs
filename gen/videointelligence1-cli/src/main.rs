...
impl<'n> Engine<'n> {
    ...
    async fn _projects_locations_corpura_operations_get(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
            ...
            ("projects", Some(opt)) => {
                match opt.subcommand() {
                    ("locations-corpura-operations-get", Some(opt)) => {
                        call_result = self._projects_locations_corpura_operations_get(opt, dry_run, &mut err).await;
                    },
