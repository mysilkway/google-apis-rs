                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           ...
                                                                           v.extend(["wait-token", "agent-id", "success-on-timeout"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           v.extend(self.gp.iter().map(|v|*v));
                                                                           v.extend(["include-all-users", "action-value", "client-version", "wait-token", "strip-results", "include-inactive"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
