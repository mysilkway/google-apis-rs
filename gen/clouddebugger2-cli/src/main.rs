...
extern crate mime;
extern crate strsim;
extern crate google_clouddebugger2;
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::CloudDebugger<hyper::Client<hyper_rustls::HttpsConnector<hyper::client::connect::HttpConnector>, hyper::body::Body>, Authenticator<DefaultAuthenticatorDelegate, JsonTokenStorage, hyper::Client<hyper_rustls::HttpsConnector<hyper::client::connect::HttpConnector>, hyper::body::Body>>>,
    gp: Vec<&'static str>,
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           ...
                                                                           v.extend(["wait-token", "success-on-timeout", "agent-id"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           v.extend(self.gp.iter().map(|v|*v));
                                                                           v.extend(["wait-token", "strip-results", "client-version", "include-inactive", "include-all-users", "action-value"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
