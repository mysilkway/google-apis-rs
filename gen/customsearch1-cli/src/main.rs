...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::Customsearch<hyper::Client<hyper_rustls::HttpsConnector<hyper::client::connect::HttpConnector>, hyper::body::Body>
    >,
    gp: Vec<&'static str>,
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           ...
                                                                           v.extend(["or-terms", "sort", "hl", "c2coff", "num", "img-size", "hq", "cx", "rights", "site-search", "link-site", "low-range", "start", "safe", "lr", "img-color-type", "img-type", "search-type", "cr", "filter", "exclude-terms", "high-range", "q", "gl", "date-restrict", "file-type", "exact-terms", "googlehost", "img-dominant-color", "related-site", "site-search-filter"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
